(function() {
  tinymce.create('tinymce.plugins.IcePlugin', {
    ...
    pluginPath: undefined,                    // path of the plugin; if not defined, plugin is supposed to be in tinymce plugins subfolder
    ...
    init: function(ed, url) {
      ...
      ed.on('keydown', function(e) {
        ...
        if((e.keyCode == ice.dom.DOM_VK_DELETE || e.keyCode == ice.dom.DOM_VK_BACK_SPACE) && self.isTracking) {
          e.preventDefault();
        }
      });
      ...
      ed.on('init', function(e) {
        ...
        self.realUrl = url;
        if (self.pluginPath) {
          self.realUrl = this.documentBaseUrl + self.pluginPath;
        }

        self.insertSelector = '.' + self.insertClass + '[' + self.changeIdAttribute + ']';
        self.deleteSelector = '.' + self.deleteClass + '[' + self.changeIdAttribute + ']';
        ...
        // reference css for editor content
        dom.loadCSS(self.css.indexOf('://') > 0 ? self.css : (self.realUrl + '/' + self.css));

        // regerence css for context menu (context menu is appended in
        // main document, therefore styles must be added to main document
        var maindoc = parent.document;
        var head = maindoc.getElementsByTagName('head')[0];
        var style = maindoc.createElement('style');
        style.type = 'text/css';
        style.innerHTML = ".mce-i-accept { background-image: url("+self.realUrl+"/img/accept.svg) !important; } " +
                          ".mce-i-reject { background-image: url("+self.realUrl+"/img/reject.svg) !important; } " +
                          ".mce-i-ice-accept { background-image: url("+self.realUrl+"/img/ice-accept.svg) !important; } " +
                          ".mce-i-ice-reject { background-image: url("+self.realUrl+"/img/ice-reject.svg) !important; } " +
                          ".mce-i-ice-showchanges { background-image: url("+self.realUrl+"/img/ice-showchanges.svg) !important; } " +
                          ".mce-i-ice-togglechanges { background-image: url("+self.realUrl+"/img/ice-togglechanges.svg) !important; } " +
                          ".mce-i-ice-accept-change { background-image: url("+self.realUrl+"/img/ice-accept-change.svg) !important; } " +
                          ".mce-i-ice-reject-change { background-image: url("+self.realUrl+"/img/ice-reject-change.svg) !important; } ";
        head.appendChild(style);

        if (self.runMode != 'demo') {
          // note: we don't explicitly load the ice.min.js in demo mode
          // since ice.js is already linked in demo/index.html
          /**
           * TODO/FIXME - Investigate further into why this doesn't work...
           *
          tinymce.ScriptLoader.load(self.realUrl + '/js/ice.min.js', function() {
          ...
          script.src = self.realUrl + '/js/ice.min.js?version='+self.scriptVersion;
      ...
      ed.addCommand('initializeice', function(editor) {
        ...
        tinymce.DOM.win.setTimeout(function() {
          changeEditor = new ice.InlineChangeEditor({
            ...
            spaceChar: ' ',
            titleDateFormat: function(time) {
              var d = new Date(time);
              var l2 = function(x) { return ('00'+x).substring(String(x).length); }
          ...
          if (/(^|[ ,])(power)?paste([, ]|$)/.test(ed.settings.plugins) ||
              ed.plugins.paste || ed.plugins.powerpaste) {
            var txt = "Simultaneous usage of 'ICE' and 'paste' or 'powerpaste' plugins is not recommended.<br/>"+
                      "You may experience unattended behaviour while pasting text.<br/>"+
                      "Please remove 'paste' and/or 'powerpaste' from 'plugins' option.";
          ...
          ed.on('cut', function(e) {
            if (jQuery(e.target).prop('tagName') == 'DIV' &&
                jQuery(e.target).attr('id') == changeEditor.pluginsManager.plugins['IceCopyPastePlugin']._cutId) {
              // let's keyPress handle this event
              return true;
            } else {
              return changeEditor.handleEvent(e);
            }
      ...
      ed.addCommand('ice_toggleshowchanges', function() {
        ...
        if(ed.dom.hasClass(body,'CT-hide')) {
          ...
          if (ed.plugins.ice.showChangesButton.active)
        ...
        if (ed.plugins.ice.acceptAllButton.disabled)
          ed.plugins.ice.acceptAllButton.disabled(disabled);
        if (ed.plugins.ice.rejectAllButton.disabled)
          ed.plugins.ice.rejectAllButton.disabled(disabled);
        if (ed.plugins.ice.acceptButton.disabled)
          ed.plugins.ice.acceptButton.disabled(disabled);
        if (ed.plugins.ice.rejectButton.disabled)
      ...
      ed.addCommand('ice_enable', function() {
        ...
        if (ed.plugins.ice.trackChangesButton.active)
          ed.plugins.ice.trackChangesButton.active(true);
        if (ed.plugins.ice.showChangesButton.disabled)
      ...
      ed.addMenuItem('iceaccept', {
        ...
        onPostRender: function(event) {
          var self = this;
          ed.on('NodeChange', function (e) {
            self.visible(isInsideChangeTag(e.element));
          });
          // onPostReneder is evaluated only once when contextmenu is
          // opened. But this occures after node selection has changed.
          // Therefore the 'NodeChange' event above won't be executed on
          // the very first time contextmenu is open. We force the
          // visibility status for this very first time.
          this.visible(isInsideChangeTag(ed.selection.getNode()));
        }
      });
      ...
      ed.addMenuItem('icereject', {
        ...
        onPostRender: function(event) {
          ...
          // see comment for iceaccept above
          this.visible(isInsideChangeTag(ed.selection.getNode()));
        }
      });
